<template>
  <div>
        <div class="d-flex mb-2">
          ...
          <select
            class="form-control flex-grow-1 mx-3"
            value="fetchIssue.id"
            @change="selectIssue"
          >
            <option value="0">-- Выберите выпуск --</option>
            <option v-for="(issue, index) in issues" :key="index"
                    :value="issue.id">Год: {{issue.year}}, Номер {{issue.no}} ({{issue.full_no}}), Часть {{issue.part}}
            </option>
