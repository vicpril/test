...
class SpaController extends SiteController
{
    ...
    public function index()
    {
        ...
        //     dump("YEAR: $year");
        //     $this->importPosts($year);
        // }
        $this->importIssueTitleFiles();
...
//         dump($post->title_en);
    ...
    public function importIssueTitleFiles()
    {
        $posts = wpPost::where('post_type', 'post')
            ->status('publish')
        // this is for title pages paths - run in the end
            ->hasMeta('_edit_last')
    ...
    public function importPosts($year)
    {
        $posts = wpPost::where('post_type', 'post')
            ->status('publish')
        // this is for title pages paths - run in the end
        ...
        foreach ($posts as $post) {
            ...
            try {
                if ($cat) {
                    $cat = Category::where('alias', $cat->slug)->first();
                    $cat_id = $cat->id;
                    $cat->touchUsed();
                } else {
                    $cat = Category::where('alias', 'no-rubrik')->first();
                    $cat_id = $cat->id;
                    $cat->touchUsed();
                }
            } catch (\Throwable $th) {
